...
public class CircuitBreaker extends BusModBase {
  ...
  public void start() {
    ...
    final AtomicInteger todo = new AtomicInteger( circuits.getFieldNames().size() ) ;
    ...
    for( final String name : circuits.getFieldNames() ) {
      ...
      container.deployModule( name, modConfig, instances, new AsyncResultHandler<String>() {
        public void handle( AsyncResult<String> response ) {
          ...
          container.deployVerticle( CircuitBreakerProxy.class.getName(), circuitConfig, new AsyncResultHandler<String>() { 
            public void handle( AsyncResult<String> response ) {
              if( todo.decrementAndGet() == 0 ) {
                eb.send( String.format( "%s.ready", CircuitBreaker.this.address ), new JsonObject() {{
                  putString( "status", "ok" ) ;
                }} ) ;
              }
            }
